...
function getTableOfContentsMarkdown(schema, orderedTypes, headerLevel) {
  ...
  for (var title in orderedTypes) {
    ...
    if (
      title === schema.title ||
      currentType.parents.length > 1 ||
      currentType.parents.indexOf(schema.title) !== -1
    ) {
      ...
      const item =
        style.getTOCLink(title, typeName) +
        (title === schema.title ? " (root object)" : "");
...
function getRecursiveTOC(orderedTypes, parentTitle, depth) {
  ...
  for (var i = 0; i < orderedTypes[parentTitle].children.length; i++) {
    ...
    if (orderedTypes[currentTitle].parents.length === 1) {
      ...
      const item = style.getTOCLink(
        currentTitle.replace(parentTitle + " ", ""),
        typeName
      );
...
function getSchemaMarkdown(
  ...
  if (defined(extendedDescription)) {
    md +=
      autoLinkDescription(extendedDescription, knownTypes, autoLink) + "\n\n";
  }
  ...
  if (defined(webgl)) {
    md +=
      style.propertyGltfWebGL("Related WebGL functions") +
      ": " +
      webgl +
      "\n\n";
  }
  if (schemaType === "string") {
    var enumString = getEnumString(schema, schema.title, 1);
    if (defined(enumString)) {
      enumString =
        " **Allowed values** :  \n" +
        enumString.replace(/(?:\ \ \ \ \*)/g,"-");
      md += enumString;
    }
  }
  ...
  if (schemaType === "object") {
    ...
    if (embedMode === enums.embedMode.referenceIncludeDocument) {
      md +=
        style.bulletItem(
          style.bold("JSON schema") +
            ": " +
            style.getSchemaEmbedLink(fileName, schema)
        ) + "\n";
      ...
      md +=
        style.bulletItem(
          style.bold("JSON schema") +
            ": " +
            style.getLinkMarkdown(
              fileName,
              schemaRelativeBasePath.replace(/\\/g, "/") + fileName
            )
        ) + "\n";
    }
    ...
    if (!summaryOnly) {
      var title = defaultValue(
        schema.title,
        suppressWarnings ? "" : "WETZEL_WARNING: title not defined"
      );
      md += createPropertiesDetails(
        schema,
        title,
        headerLevel + 1,
        knownTypes,
...
function createPropertiesSummary(schema, knownTypes, autoLink, describeEnums) {
  ...
  if (
    ...
    md += style.beginTable(style.typeValue(schema.title) + " Properties", [
      "   ",
      "Type",
      "Description",
      "Required",
    ...
    for (var name in properties) {
      if (properties.hasOwnProperty(name)) {
        ...
        md += style.addTableRow([
          ...
          description,
          (summary.required === "Yes" ? style.requiredIcon : "") +
            summary.required,
        ]);
      }
    }
...
function createPropertiesDetails(
  schema,
  title,
  headerLevel,
  knownTypes,
  ...
  for (var name in properties) {
    if (properties.hasOwnProperty(name)) {
      ...
      // TODO: Add plugin point for custom JSON schema properties like gltf_*
      var detailedDescription = autoLinkDescription(
      ...
      var eachElementInTheArrayMust =
        "Each element in the array" + style.mustKeyword;
      ...
      if (defined(items)) {
        ...
        var itemsExclusiveMinimum =
          defined(items.exclusiveMinimum) && items.exclusiveMinimum;
        var minString = itemsExclusiveMinimum
          ? "greater than"
          : "greater than or equal to";

        var itemsExclusiveMaximum =
          defined(items.exclusiveMaximum) && items.exclusiveMaximum;
        var maxString = itemsExclusiveMaximum
          ? "less than"
        ...
        if (defined(items.minimum) && defined(items.maximum)) {
          md += style.bulletItem(
            eachElementInTheArrayMust +
              ...
              minString +
              " " +
              style.minMax(items.minimum) +
              " and " +
              maxString +
              " " +
              style.minMax(items.maximum) +
              ".",
            1
          );
        ...
        if (defined(items.minLength) && defined(items.maxLength)) {
          md += style.bulletItem(
            eachElementInTheArrayMust +
              "have length between " +
              style.minMax(items.minLength) +
              " and " +
              style.minMax(items.maxLength) +
              ".",
            1
          );
          ...
          md += style.bulletItem(
            eachElementInTheArrayMust +
              "have length greater than or equal to " +
              style.minMax(items.minLength) +
              ".",
            1
          );
          ...
          md += style.bulletItem(
            eachElementInTheArrayMust +
              "have length less than or equal to " +
              style.minMax(items.maxLength) +
              ".",
            1
          );
      ...
      if (
        ...
        md += style.bulletItem(
          style.propertyDetails("Minimum") +
            ": " +
            style.minMax(" > " + property.exclusiveMinimum),
          0
        );
      } else {
        ...
        if (defined(minimum)) {
          ...
          md += style.bulletItem(
            style.propertyDetails("Minimum") +
              ": " +
              style.minMax((exclusiveMinimum ? " > " : " >= ") + minimum),
            0
          );
        }
      }
      ...
      if (
        ...
        md += style.bulletItem(
          style.propertyDetails("Maximum") +
            ": " +
            style.minMax(" < " + property.exclusiveMaximum),
          0
        );
      } else {
        ...
        if (defined(maximum)) {
          ...
          md += style.bulletItem(
            style.propertyDetails("Maximum") +
              ": " +
              style.minMax((exclusiveMaximum ? " < " : " <= ") + maximum),
            0
          );
        }
      }
      ...
      if (defined(format)) {
        md += style.bulletItem(
          style.propertyDetails("Format") + ": " + format,
          0
        );
      }
      ...
      if (defined(pattern)) {
        md += style.bulletItem(
          style.propertyDetails("Pattern") + ": " + style.minMax(pattern),
          0
        );
      }
      ...
      if (defined(minLength)) {
        md += style.bulletItem(
          style.propertyDetails("Minimum Length") +
            style.minMax(": >= " + minLength),
          0
        );
      }
      ...
      if (defined(maxLength)) {
        md += style.bulletItem(
          style.propertyDetails("Maximum Length") +
            style.minMax(": <= " + maxLength),
          0
        );
      }
      ...
      if (defined(enumString)) {
        md +=
          style.bulletItem(style.propertyDetails("Allowed values") + ":", 0) +
          enumString;
      }
      ...
      if (
        ...
        if (defined(additionalPropertiesType)) {
          ...
          if (
            ...
            defined(additionalProperties.title)
          ) {
            formattedType = style.linkType(
          ...
          md += style.bulletItem(
            style.propertyDetails("Type of each property") +
              ": " +
              formattedType,
            0
          );
      ...
      if (defined(webgl)) {
        md += style.bulletItem(
          style.propertyGltfWebGL("Related WebGL functions") + ": " + webgl,
          0
        );
      }
...
function getPropertySummary(property, knownTypes, autoLink) {
  ...
  if (type === "array") {
    ...
    if (defined(property.items) && defined(property.items.type)) {
      if (property.items.type === "object") {
        ...
        formattedType =
          style.linkType(style.typeValue(type), type, autoLink) + " ";
  ...
  var description = autoLinkDescription(
